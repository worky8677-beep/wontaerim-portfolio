...
export function ExperienceItem({ start, end, title, description, isFirst = false }) {
  return (
    <div className="flex flex-col gap-2">
      <div className={`inline-flex gap-[5px] items-center px-[10px] py-[2px] rounded-[10px] self-start ${isFirst ? "bg-snowflake" : "bg-alabaster"}`}>
        <div className="size-2 overflow-hidden">
          <img src={isFirst ? asteriskGreen : asteriskGrey} alt="" className="size-full" />
        </div>
      ...
      <div className="flex flex-col gap-2">
        <p className="font-gmarket font-semibold text-body1 text-black">{title}</p>
        {description && (
          <p className="font-gmarket text-body3 text-dove leading-6">{description}</p>
        )}
      </div>
    </div>
  );
